// Shared staff-facing "solicitud de diseño" card — the single source of
// truth for claiming, copying the AI prompt, delivering, and rejecting a
// design request. Used by both /witer (designers) and /admin's Solicitudes
// tab, so an admin claiming or copying a prompt gets the exact same
// experience (and feedback) a designer does, instead of two independently
// maintained versions drifting apart. Admin-only extras (seeing the
// client's name/email, reactivating a post-close change request) are
// opt-in via the `isAdmin` prop — a designer's data never has the contact
// fields populated anyway (see /api/designer/requests), and only an admin
// account can call /api/admin/activate-change server-side regardless.
...
export type StaffRequestRow = {
  ...
  // Admin-only — never present on a designer's own view of a request
  // (/api/designer/requests deliberately omits these).
  user_name?: string;
  user_email?: string;
};
...
// prompt" button was clicked, instead of a page-level toast — so whoever
// clicked doesn't have to look away from what they just clicked.
...
// Collapsed row for a pending request not yet claimed — title/format/date
// only, no client info, no copy-prompt button, to avoid showing full detail
// on work nobody has picked up yet. The glowing ring only appears while
// it's unclaimed (a nudge that it needs someone); once claimed the parent
// swaps this out for the full StaffRequestCard.
export function PendingCompactCard({
  row,
  me,
  queryKey,
}: {
...
// Collapsed row for a request that no longer needs action — delivered (en
// revisión del cliente), rejected, or closed. Title, date, and the
// delivered thumbnail only — clicking it expands into the full
// StaffRequestCard for detail instead of always showing every field.
  ...
  if (expanded) {
    return (
      <div>
        ...
        <StaffRequestCard row={row} me={me} onSent={onSent} queryKey={queryKey} isAdmin={isAdmin} />
      </div>
    );
  }
    ...
    <article className="wit-glass rounded-2xl p-6 shadow-[0_10px_30px_rgba(5,13,40,0.05)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-wit-gray">
            {isAdmin && row.user_name ? `${row.user_name} (${row.user_email}) · ` : ""}
            {row.aspect_ratio}
            {row.style ? ` · ${row.style}` : ""} ·{" "}
            {new Date(row.created_at + "Z").toLocaleString("es-MX")}
          </p>
      ...
      {row.revision_note_1 || row.revision_note_2 ? (
        <div className="mt-3 space-y-2">
          {[
            ...
            // change has been requested, it's already handled: lock it so
            // nobody re-copies an old prompt by mistake.
      ...
      {isAdmin && row.status === "cambio_solicitado" ? (
