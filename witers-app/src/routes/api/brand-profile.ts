  ...
  server: {
    handlers: {
      ...
      PATCH: async ({ request }) => {
        ...
        if (!parsed.success) return json({ ok: false, error: "datos_invalidos" }, { status: 400 });
        await db()
          .prepare("UPDATE brand_assets SET use_in_planning = ?3 WHERE id = ?1 AND user_id = ?2")
          .bind(parsed.data.id, user.id, parsed.data.enabled ? 1 : 0)
          .run();
        return json({ ok: true });
      },
      DELETE: async ({ request }) => {
        ...
        if (!parsed.success) return json({ ok: false, error: "datos_invalidos" }, { status: 400 });
        await db()
          .prepare("DELETE FROM brand_assets WHERE id = ?1 AND user_id = ?2")
