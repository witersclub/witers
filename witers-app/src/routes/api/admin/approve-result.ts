import { createFileRoute } from "@tanstack/react-router";
import { z } from "zod";
import process from "node:process";

import { db, json, requireAdminUser } from "../../../lib/witers-auth.server";

const schema = z.object({
  requestId: z.string().uuid(),
  code: z.string().min(1).max(200),
});

// Admin-only: approve AI-generated drafts (kind='draft') for a request so
// they become visible to the client. Requires ADMIN_APPROVAL_CODE secret:
//   set it under Workers → Settings → Variables and secrets (Runtime).
export const Route = createFileRoute("/api/admin/approve-result")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const auth = await requireAdminUser(request);
        if (!auth.ok) return json({ ok: false, error: "no_admin" }, { status: auth.status });

        const expectedCode = process.env.ADMIN_APPROVAL_CODE;
        if (!expectedCode) {
          return json({ ok: false, error: "falta_codigo_aprobacion" }, { status: 500 });
        }

        const parsed = schema.safeParse(await request.json().catch(() => null));
        if (!parsed.success) {
          return json({ ok: false, error: "datos_invalidos" }, { status: 400 });
        }

        if (parsed.data.code !== expectedCode) {
          return json({ ok: false, error: "codigo_incorrecto" }, { status: 403 });
        }

        const result = await db()
          .prepare(
            "UPDATE request_results SET kind = 'generated' WHERE request_id = ?1 AND kind = 'draft'",
          )
          .bind(parsed.data.requestId)
          .run();

        if (!result.meta.changes) {
          return json({ ok: false, error: "sin_borradores" }, { status: 404 });
        }

        await db()
          .prepare(
            "UPDATE design_requests SET status = 'completada', updated_at = datetime('now') WHERE id = ?1",
          )
          .bind(parsed.data.requestId)
          .run();

        return json({ ok: true });
      },
    },
  },
});
